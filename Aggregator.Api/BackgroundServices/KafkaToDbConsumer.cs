...
public class KafkaToDbConsumer : BackgroundService
{
        ...
        ILogger<KafkaToDbConsumer> logger)
    {
        ...
        _minBatchSize = configuration.GetValue<int>("Kafka:MinBatchSize");
        _maxBatchSize = configuration.GetValue<int>("Kafka:MaxBatchSize");
        _batchInterval = configuration.GetValue<TimeSpan>("Kafka:BatchInterval");
